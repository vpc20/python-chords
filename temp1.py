...
def chord_id(reformatted_chord):
    ...
    orig_chord, reformatted_chord = reformat_chord(reformatted_chord)
    chord_inversions = [' '.join(perms) for perms in permutations(reformatted_chord.split())]
    for chord_inv in chord_inversions:
        chord_formula = gen_chord_formula(chord_inv)
        # print(chord_inv, '-->', chord_formula)
        if chord_formula in chord_dict:
            inv_root_note = chord_inv.split()[0]
            if len(inv_root_note) == 2:  # flatted note
                chord_name = inv_root_note[0].upper() + inv_root_note[1] + chord_dict[chord_formula]
            else:
                chord_name = inv_root_note[0].upper() + chord_dict[chord_formula]
            if inv_root_note != reformatted_chord.split()[0]:  # slash chords
                orig_root_note = orig_chord.split()[0]
                if len(orig_root_note) == 2:
                    chord_name = chord_name + '/' + orig_root_note[0].upper() + orig_root_note[1]
                else:
                    chord_name = chord_name + '/' + orig_root_note[0].upper()
